...
class HumanoidLongJump(PMBase):
    ...
    def set_initial_root_state(self):
        initial_humanoid_root_states = self._initial_humanoid_root_states.clone()
        ...
        # Apply changes
        self._initial_humanoid_root_states = initial_humanoid_root_states
    ...
    def _reset_actors(self, env_ids):
        ...
        root_rot = self._initial_humanoid_root_states[env_ids, 3:7].clone()
        root_vel = self._initial_humanoid_root_states[env_ids, 7:10].clone() * 0.0
        root_ang_vel = self._initial_humanoid_root_states[env_ids, 10:13].clone() * 0.0
    ...
    def compute_failures_and_distances(self):
        ...
        x_over_40 = torch.any(self._rigid_body_pos[:, self.non_termination_contact_body_ids, 0] > jump_start_x[:, None],
                              dim=-1)  # shape 1024
        ...
        contact_force_not_zero = torch.sqrt(
            torch.sum(torch.sum(torch.square(self._contact_forces), dim=-1), dim=-1)) > force_threshold
        ...
        self._jump_length[reset_x_over_40_and_contact_force_not_zero] = torch.mean(
            self._rigid_body_pos[reset_x_over_40_and_contact_force_not_zero][:, :, 0],
    ...
    def _reset_task(self, env_ids=None):
        if len(env_ids) > 0:
    ...
    def _compute_reset(self):
        ...
        self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_reset(self.reset_buf,
                                                                           self.progress_buf,
                                                                           self._contact_forces,
                                                                           self.non_termination_contact_body_ids,
                                                                           self._rigid_body_pos,
